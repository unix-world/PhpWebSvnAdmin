...
// bug fixes by unixman, 2016-08-08:
//		* on newer PHP versions Geshi fail silently ... so use the PHP's highlight_string() instead
...
class SVNRepository {
	...
	function svnCommandString($command, $path, $rev, $peg) {
		global $config;
		$cmd = $config->getSvnCommand().$this->repConfig->svnCredentials().' '.$command.' '.($rev ? '-r '.$rev.' ' : '').quote(encodePath($this->getSvnPath($path)).'@'.($peg ? $peg : ''));
		//echo $cmd; die(); // aaa
		//file_put_contents('_cmds-log.txt', $cmd."\n", FILE_APPEND | LOCK_EX);
		return (string) $cmd;
	}
	...
	function applyGeshi($path, $filename, $language, $rev, $peg = '', $return = false) {
		...
		//-- bug fix by unixman: on newer PHP versions Geshi fail silently ... so use the PHP's highlight_string() instead
		if((string)$language == 'php') {
			ini_set('highlight.html', '#000000'); // required !!
			$the_code = (string) @highlight_string($source, true);
			$the_code = str_replace(["\r\n", "\r"], ["\n", "\n"], $the_code);
			$the_code = str_replace( // fix for diff ; this needs PHP >= 5 and: ini_set('highlight.html', '#000000');
				['<code>'."\n".'<span style="color: #000000">', '<code><span style="color: #000000">', '</span>'."\n".'</code>', '</span></code>'],
				['', '', '', ''],
				$the_code
			);
			...
			$the_code = (string) $this->geshi->parse_code();
		} //end if else
		//-- #end fix
